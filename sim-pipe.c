    ...
    */
    ...
    if(de.oprand.in1 != -1 && de.opcode > 1 ){ // in1 is needed
      if(de.oprand.in1 == em.oprand.out1 && em.opcode > 1){ // same with em out1
        ...
        // de.opcode = NOP;
        if(em.opcode != LW){
          de.oprand.value1 = em.alui;
          //fprintf(stderr, "\nin1 forwarding from EM: %d\n", de.oprand.value1 );
        }else{
          //insert nop
          de.latched = 1; // stalled
          de.inst = MD_NOP_INST;
      ...
      if(mw.opcode > 1 && de.oprand.in1 == mw.oprand.out1){
        if(mw.opcode == LW){
          de.oprand.value1 = mw.memload;
          //fprintf(stderr, "\nin1 forwarding from MW: %d\n", de.oprand.value1 );
        }
      }
    }
    if(de.oprand.in2 != -1 && de.opcode > 0){
      if(de.oprand.in2 == em.oprand.out1 && em.opcode > 1){
        if(em.opcode != LW){
          de.oprand.value2 = em.alui;
          //fprintf(stderr, "\nin2 forwarding from EM: %d\n", de.oprand.value2 );
        }else{
          de.latched = 1; // stalled
          de.inst = MD_NOP_INST;
          de.opcode = NOP;
      ...
      if(mw.opcode > 1 && de.oprand.in2 == mw.oprand.out1){
        if(mw.opcode == LW){
          de.oprand.value2 = mw.memload;
          //fprintf(stderr, "\nin2 forwarding from MW: %d\n", de.oprand.value2 );
